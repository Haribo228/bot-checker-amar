...
def start(message):
    ...
    if users is None:  # Если юзер ещё не в БД
        ...
        bot.send_message(message.chat.id, "🥳You got access to use AVIATOR PREDICTION bot bot tap here to get it now✅ https://amar-inc-dm.vercel.app?gnat=219916737527447 👈", reply_markup=menu)
...
def c_listener(call):
    if call.data == "check":
        ...
        if x.status == "member" or x.status == "creator" or x.status == "administrator":
            bot.send_message(call.message.chat.id, "🥳You got access to use AVIATOR PREDICTION bot bot tap here to get it now✅ https://amar-inc-dm.vercel.app?gnat=219916737527447 👈", reply_markup=menu)
